...
const bignum = require('paillier-js/node_modules/big-integer');
...
const BVoting = artifacts.require('../contracts/BVoting.sol')
...
contract('BVotingVerifier', (accounts) => {
    ...
    describe('Install contracts and test', () => {
        beforeEach(async () => {
            ...
            bvotingVerifier = await Verifier.new(accounts[0]);
            verifierAddress = await bvotingVerifier.address;
            bvoting = await BVoting.new(web3.utils.toHex('ballot0001'), web3.utils.toHex(_publicKey), verifierAddress);
            bvotingABI = await bvoting.abi;
            bvotingAddress = await bvoting.address;
            allAccounts = accounts;
        });

        it('Test BVoting contract', async () => {
            result = await bvoting.ballotIdentifier().then(function (res) {
                return (res);
            })
        ...
        it("Verify zk vote proof on smartcontract", async () => {
            ...
            result = await bvotingVerifier.verifyProof.call(verifyCall.a, verifyCall.ap, verifyCall.b, verifyCall.bp, verifyCall.c, verifyCall.cp, verifyCall.h, verifyCall.kp, verifyCall.inputs);
...
describe("Create and test an ethereum stealth wallet", () => {
    ...
    it("Organiser receives compressedPubScanKeys from voter, creates a random stealth wallet and send to smart-contract", async () => {
        ...
        console.log('StealthAddress Args', ethStealthAddress, web3.utils.toHex(pubKeyToRecover), web3.utils.toHex(opMarker));
        let canVote = await bvoting.ephemeralVoters(ethStealthAddress);
        ...
        await bvoting.addEphemeralVoter(ethStealthAddress, web3.utils.toHex(pubKeyToRecover), web3.utils.toHex(opMarker))
            .then(async () => {
    ...
    it("Organiser checks if stealth wallet can vote", async () => {
        ...
        let canVote = await bvoting.ephemeralVoterArray(await bvoting.ephemeralVoters(ethStealthAddress));
    ...
    it("Voter discovers his stealth wallet", async () => {
        ...
        let ewCount = await bvoting.votersCount();

        for (var i = 0; i < ewCount; i++) {
            ew = await bvoting.getEphemeralWallets(i);
...
describe("Encrypt, count, decrypt and test votes result proof", () => {
    ...
    it("Encrypt votes", async () => {
        ...
        // let result = await bvoting.encryptionPublicKey().then(function (res) {
        //     return (web3.utils.hexToAscii(res));
        // })
        // const _publicKey = JSON.parse(result);
        publicKey = new paillier.PublicKey(bigInt(_publicKey.n), bigInt(_publicKey.g));
        assert(publicKey.bitLength == '1024')

        for (let i = 0; i < votes.president.length; i++) {
            voteCount += votes.president[i];
            // convert vote to bignum
            //let bn1 = bigInt(votes.president[i]).mod(publicKey.n);
            let bn1 = bigInt(votes.president[i]);
            ...
            votes.president[i] = publicKey.encrypt(bigInt(votes.president[i]));
        }

        for (let j = 0; j < votes.senator.length; j++) {
            voteCount += votes.senator[j];
            // convert vote to bignum
            let bn2 = bigInt(votes.senator[j]).mod(publicKey.n);
        ...
        for (let k = 0; k < votes.stateGovernor.length; k++) {
            ...
            // convert vote to bignum
            let bn3 = bigInt(votes.stateGovernor[k]).mod(publicKey.n);
    ...
    it("Register encrypted votes on blockchain through ephemeral wallet", async () => {
        ...
        let canVote = await bvoting.ephemeralVoterArray(await bvoting.ephemeralVoters(ethAddress));
        assert.isTrue(canVote.canVote);
        ...
        // Could be transformed back using bigInt(web3.utils.hexToAscii(hexVote))
        ...
        const contract = new web3.eth.Contract(bvotingABI, bvotingAddress);
        console.log("Voted By: ", ethAddress)
        console.log("AddVote Args: ", hexVotes.president, hexVotes.senator, hexVotes.stateGovernor, web3.utils.toHex('commit1'))
        ...
        const estimateGas1 = await bvoting.addVote.estimateGas(hexVotes.president, hexVotes.senator, hexVotes.stateGovernor, web3.utils.toHex('commit1'), { from: ethAddress });
        ...
        canVote = await bvoting.ephemeralVoterArray(await bvoting.ephemeralVoters(ethAddress));
        assert.isNotTrue(canVote.canVote);
    ...
    it("Recovering votes from blockchain and sum all together", async () => {
        ...
        // Get total votes
        let votesCount = await bvoting.votesCount();
        ...
        privateKey = new paillier.PrivateKey(bigInt(_privateKey.lambda), bigInt(_privateKey.mu), bigInt(_privateKey.p), bigInt(_privateKey.q), bigInt(_privateKey.publicKey));
        ...
        encryptedTotalSum = publicKey.encrypt(bigInt(encryptedTotalSum));

        for (var i = 0; i < votesCount; i++) {
            bVotesArray = await bvoting.getVotes(i);
            ...
            for (let j = 0; j < bVotesArray[0].length; j++) {
                encryptedTotalSum = publicKey.addition(bigInt(web3.utils.hexToAscii(encryptedVotes[i].president[j])), encryptedTotalSum);
                if (encryptedPresidentSum[j] == null) {
                    encryptedPresidentSum[j] = 0;
                    let bn5 = bigInt(encryptedPresidentSum[j]).mod(publicKey.n);
                    while (bn5.lt(0)) bn5 = bn5.add(publicKey.n);
                    encryptedPresidentSum[j] = publicKey.encrypt(encryptedPresidentSum[j]);
                }
                encryptedPresidentSum[j] = publicKey.addition(bigInt(web3.utils.hexToAscii(encryptedVotes[i].president[j])), encryptedPresidentSum[j]);
            }

            for (let k = 0; k < bVotesArray[1].length; k++) {
                if (encryptedSenatorSum[k] == null) {
                    encryptedSenatorSum[k] = 0;
                    let bn5 = bigInt(encryptedSenatorSum[k]).mod(publicKey.n);
                    while (bn5.lt(0)) bn5 = bn5.add(publicKey.n);
                    encryptedSenatorSum[k] = publicKey.encrypt(encryptedSenatorSum[k]);
                }
                encryptedTotalSum = publicKey.addition(bigInt(web3.utils.hexToAscii(encryptedVotes[i].senator[k])), encryptedTotalSum);
                encryptedSenatorSum[k] = publicKey.addition(bigInt(web3.utils.hexToAscii(encryptedVotes[i].senator[k])), encryptedSenatorSum[k]);
            }

            for (let l = 0; l < bVotesArray[2].length; l++) {
                if (encryptedStateGovernorSum[l] == null) {
                    encryptedStateGovernorSum[l] = 0;
                    let bn5 = bigInt(encryptedStateGovernorSum[l]).mod(publicKey.n);
                    while (bn5.lt(0)) bn5 = bn5.add(publicKey.n);
                    encryptedStateGovernorSum[l] = publicKey.encrypt(encryptedStateGovernorSum[l]);
                }
                encryptedTotalSum = publicKey.addition(bigInt(web3.utils.hexToAscii(encryptedVotes[i].stateGovernor[l])), encryptedTotalSum);
                encryptedStateGovernorSum[l] = publicKey.addition(bigInt(web3.utils.hexToAscii(encryptedVotes[i].stateGovernor[l])), encryptedStateGovernorSum[l]);
            }
        }
    ...
    it("Testing sum on encrypted votes straight from memory", () => {
        ...
        while (bn5.lt(0)) bn5 = bn5.add(publicKey.n);
        encryptedSum = publicKey.encrypt(bigInt(encryptedSum));
    ...
    it("Create a proof-of-result, and test result", () => {
        ...
        console.log(result, decrypted, voteCount);
